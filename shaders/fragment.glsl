...
void main() {
    ...
    if (fragPos.y > 10.0) {
        float snowBlend = smoothstep(5.0, 40.0, fragPos.y) * slope;
    ...
    float diff = NdotL * 0.5 + 0.5; //half lambert shading preventing harsh black on back faces
    diff = diff * diff; //makes light falloff softer
